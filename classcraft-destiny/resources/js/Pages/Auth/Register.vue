            ...
            <form @submit.prevent="handleSubmit" class="space-y-6">
              ...
              <div class="flex justify-between items-center pt-8 border-t border-gray-700">
                ...
                <button
                  ...
                  :disabled="!termsAccepted || isSubmitting || !selectedClass"
                >
...
const handleSubmit = () => {
  ...
  const submitData = {
    ...form.data(),
    clase_guardian: selectedClass.value
  }
  ...
  form.post(route('register'), {
    ...
    onError: (errors) => {
      isSubmitting.value = false
      errorMessage.value = errors.correo || errors.password || errors.nombre || 'Error en el registro'
      
      // Ir al paso que contiene el error
      if (errors.nombre || errors.apellido || errors.correo || errors.password) {
        currentStep.value = 1
      } else if (errors.clase_guardian) {
        currentStep.value = 2
      }
    },
    onFinish: () => {
